...
`.github/workflows/daily-market-data.yml` runs every day at 07:30 KST (22:30 UTC) and can also be started manually. It refreshes every eligible US equity row, all 159 Korean equity rows, and all 20 crypto rows in the checked-in offline universe, validates the result, and commits changed snapshots to the default branch so the connected Vercel deployment receives them automatically.

The refresh discovers each market's latest common settled session instead of relying on a hard-coded date. At least 95% of each equity universe must agree on that session or the run fails without committing. Missing or date-mismatched rows retain their exact prior timestamp, and crypto refreshes are accepted only when all configured assets are returned. Run the same process locally with `npm run refresh:market-data`, or inspect it without writing files with `npm run refresh:market-data -- --dry-run`.
