...
- **`validate-tokens` has no escape hatch.** There is nowhere to declare "this literal is intentional" — no ignore-comment the validator honours, no sanctioned non-token form. This was flagged mid-phase (Task 3) as a property of the gate itself, with a warning that the pattern would recur. It did: a border-alignment value needs to land on exactly `-1px` to sit flush against an adjacent `1px`/`2px` border, but `-1px` is not on the spacing scale (`--space-0-5` is `2px`) and `margin-bottom` is a guarded property, so `-1px` cannot be written as a literal and cannot be expressed as `var(--space-*)` either. Both sites that hit this worked around it with `margin-bottom: calc(var(--space-0-5) / -2)` — arithmetic that produces the untokenizable value from a token that isn't actually the right size, rather than a value the scale represents:
  - `src/components/ui/ui.css:178` (commit `951e5d4`)
  - `src/components/layout/region-switcher.css:34` (commit `8fb203a`)
  - `src/components/layout/layout.css:196` (commit `8fb203a`)

  Two occurrences, not three — `src/features/stock/stock.css:67`'s `calc(var(--space-0-5) * -1) calc(var(--space-1-5) * -1)` looks similar but is a different thing: it's the codemod's general negative-literal handling (`calc(var(--token) * -1)`, added by the mid-phase negative-value repair) producing an *exact* token multiple, not a hand-written workaround for a value the scale can't reach. Closing the actual gap looks like either an ignore-comment the validator recognises (e.g. `/* validate-tokens: literal ok — border alignment */`) so a genuinely one-off value can be declared as a literal without lying about it being on the scale, or explicitly documenting `calc(var(--space-n) / k)` as the sanctioned idiom for scale-derived-but-off-scale values so it reads as policy rather than as three independent workarounds. Neither was done this phase.
